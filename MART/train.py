...
if FTRAIN:
    print("build CIFAR-B dataset :)")
    trainset.data = get_CIFARB_dataset(trainset.data.copy())
    testset.data = get_CIFARB_dataset(testset.data.copy())
...
def main():
    ...
    if MODEL_TYPE=='resnet':
        ...
        model = WideResNet(num_classes=NUM_CLASS).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        natural_acc.append(natural_err_total.cpu().numpy())
        robust_acc.append(robust_err_total.cpu().numpy())
        print('================================================================')
        
        file_name = os.path.join(log_dir, model_dir+'_train_stats.npy')
        ...
        if robust_err_total>best_robust_acc:
            best_robust_acc = robust_err_total
            torch.save(model.state_dict(),
                       os.path.join(model_dir, 'model-res-best.pt'))
            torch.save(optimizer.state_dict(),
                       os.path.join(model_dir, 'opt-res-best.tar'))
